...
class ItemControl {
  ...
  void loopInPoints(Point p(Point p)){
    itemsIndex.forEach((key) {
      for (var i = 0; i < MatrixEngine.items[key].pixelsCoordinates.length;
      i++){
        MatrixEngine.items[key].pixelsCoordinates[i] = p(MatrixEngine.items[key].pixelsCoordinates[i]);
      }
    });
  }
  ...
  void moveUp(int p) {
    loopInItems((item) {
      item.itemPosition = Point<int>(item.itemPosition.x,item.itemPosition.y-1);
    });
  }
  ...
  void moveLeft(int p) {
    loopInItems((item) {
      item.itemPosition = Point<int>(item.itemPosition.x-1,item.itemPosition.y);
    });
  }
  ...
  void rotation(){
    final r = [[3,-2],[1,2]];
    loopInPoints((p) => Point<int>( (r[0][0]*p.x) + (r[0][1]*p.y)  , (r[1][0]*p.x) + (r[1][1]*p.y)  ));
